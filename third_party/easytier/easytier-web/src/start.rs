...
pub async fn start_web(cfg: WebConfig) -> anyhow::Result<RunningEasyTierWeb> {
    ...
    tokio::spawn(async move {
        // keep the AbortOnDropHandles alive until shutdown is requested;
        // dropping them (on cancellation) aborts the axum server tasks
        let api_task = tokio::spawn(async move {
            let _tasks = restful_server.start().await;
        });
        tokio::select! {
