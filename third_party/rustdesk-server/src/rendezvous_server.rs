...
impl RendezvousServer {
    pub fn start(port: i32, serial: i32, key: &str, rmem: usize) -> ResultType<()> {
        // sync convenience wrapper from the binary era; embedding callers use
        // the async start_with_bind directly
        let rt = hbb_common::tokio::runtime::Builder::new_multi_thread().enable_all().build()?;
        rt.block_on(Self::start_with_bind(None, port, serial, key, rmem))
    }
